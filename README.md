...
- `nvidia/Qwen3-30B-A3B-NVFP4` on a vLLM nightly picks `FLASHINFER_CUTLASS` for the NVFP4 MoE on sm_120 and is then **not deterministic at temperature 0**: six identical requests return six distinct top-20 logprob signatures, and the greedy text diverges between batches. Passing `use_fused_finalize=False` to the fused MoE call makes the same six requests bit-identical, which pins it to that one argument rather than to the checkpoint or the model. [Reproduction.](https://github.com/vllm-project/vllm/issues/54945#issuecomment-5526092139)
- llama.cpp `--spec-type draft-mtp` gives 1.77x on a Qwen3.8-27B Q4_K_M file here (67.7 to 119.8 t/s, acceptance 0.470) and only 1.16x on the 9B file, and the spread by prompt is wider than the spread by model: 1.30x to 1.41x on prose against 2.18x to 2.38x on code. Builds between #28159 and #28173 cannot start `draft-mtp` at all on a file carrying an MTP head; the first release with the fix is b10749. [Measurements.](https://github.com/ggml-org/llama.cpp/issues/28196#issuecomment-5530376324)
